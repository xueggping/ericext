...
public class ExtComponentImpl implements ExtComponentInter{
	...
	public Map<String, Object> add(ExtComponent extComponent) {
		...
		if(StringUtils.isNotEmpty(extComponent.getId())){
	...
	public Page<ExtComponent> queryByPage(Integer start, Integer limit,
		...
		StringBuilder sb = new StringBuilder(
				" SELECT A.*,(B.containerType+'_'+B.xtype + CASE WHEN B.region is null THEN '' WHEN B.region is not null THEN '_' + B.region END ) as componentname FROM WF_COMPONENT A LEFT JOIN WF_FUNCTION_PAGE B ON A.PAGEID=B.ID" +
				" WHERE 1 = 1 " );
		if(st.stringNotNull(extComponent.getPageId())){
			sb.append(" AND PageId = '"+extComponent.getPageId()+"'");
		}
		sb.append(" order by pageId,ordered");
	...
	public List<ExtComponent> queryById(String Id) {
		String sql = "SELECT *,'' as componentname from WF_COMPONENT where Id=? order by ordered";
		return ur.queryAllCustom(sql,extComponentRowMapper,Id);
	}
