$(function(){
	var userDefinedList = ['/dateTime/DateTime.js'];
	for(var i in userDefinedList){
		var newJs =  document.createElement("script");
		newJs.setAttribute("type","text/javascript");
		newJs.setAttribute("src",getRootPath() + "/script/ext/userDefined"+userDefinedList[i]);// 在这里引入了a.js
...
Ext.define("Ext.locale.zh_CN.panel.Panel", {
	...
	expandToolText : '展开此选项卡',
	collapseToolText : "收起此选项卡"
});
...
Ext.Ajax.on('requestcomplete',function(conn,response,options) {   
   if(response && response.getResponseHeader && response.getResponseHeader('_timeout')){  
       Ext.Msg.alert('提示', '会话超时，请重新登录!', function(){
           ...
           window.location = localhostPaht+projectName;   
       });  
	}
}); 