 ...
 *
 */
Ext.define('Ext.ux.grid.plugin.Exporter', {
    alias: [
        'plugin.gridexporter'
    ],
    ...
    prepareData: function(config){
		...
		if(config.exportAll && config.url){
			group = me.extractAllGroups(config.totalData,grid.getColumnManager().getColumns());
		}else{
    ...
    extractAllGroups: function(data,columns){
		var len = data.length,
		lenCols = columns.length,
		group = {
            rows: []
